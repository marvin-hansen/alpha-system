...
fn main() {
    ...
    let encoded = str_to_int(input).expect("Failed to encode string");
    println!("Encoded as integer: {}", encoded);

    // Convert back to string
    let decoded = int_to_str(encoded).expect("Failed to decode integer");
    println!("Decoded string: {}", decoded);
    assert_eq!(input, decoded);

    // Handling empty strings
    let empty = "";
    let encoded_empty = str_to_int(empty).expect("Failed to encode empty string");
    let decoded_empty = int_to_str(encoded_empty).expect("Failed to decode empty string");
    ...
    let encoded_max = str_to_int(max_length).expect("Failed to encode max length string");
    let decoded_max = int_to_str(encoded_max).expect("Failed to decode max length string");
    ...
    match int_to_str(invalid_encoded) {
