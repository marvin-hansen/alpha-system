...
fn test_time_hms() {
    ...
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7).unwrap().with_hour(24),
        None
    );
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7)
            .unwrap()
            .with_hour(u32::MAX),
        None
    );
    ...
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7).unwrap().with_minute(60),
        None
    );
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7)
    ...
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7).unwrap().with_second(60),
        None
    );
    assert_eq!(
        NaiveTime::from_hms_opt(3, 5, 7)
...
fn test_time_add() {
    ...
    check!(
        hmsm(3, 5, 59, 1_300),
        TimeDelta::try_milliseconds(-1800).unwrap(),
        hmsm(3, 5, 58, 500)
    );
    check!(
        hmsm(3, 5, 59, 1_300),
        TimeDelta::try_milliseconds(-800).unwrap(),
        hmsm(3, 5, 59, 500)
    );
    check!(
        hmsm(3, 5, 59, 1_300),
    ...
    check!(
        hmsm(3, 5, 59, 900),
        TimeDelta::try_days(12345).unwrap(),
        hmsm(3, 5, 59, 900)
    );
    check!(
        hmsm(3, 5, 59, 1_300),
    ...
    check!(
        hmsm(0, 0, 0, 0),
        TimeDelta::try_milliseconds(-990).unwrap(),
        hmsm(23, 59, 59, 10)
    );
    check!(
        hmsm(0, 0, 0, 0),
        TimeDelta::try_milliseconds(-9990).unwrap(),
        hmsm(23, 59, 50, 10)
    );
}
...
fn test_time_sub() {
    ...
    check!(
        hmsm(3, 5, 7, 900),
        hmsm(3, 5, 7, 600),
        TimeDelta::try_milliseconds(300).unwrap()
    );
    check!(
        hmsm(3, 5, 7, 200),
        hmsm(2, 4, 6, 200),
    ...
    check!(
        hmsm(3, 6, 0, 200),
        hmsm(3, 5, 59, 1_800),
        TimeDelta::try_milliseconds(400).unwrap()
    );
    ...
    assert_eq!(
        hmsm(3, 5, 6, 800) + TimeDelta::try_milliseconds(400).unwrap(),
        hmsm(3, 5, 7, 200)
    );
...
fn test_time_fmt() {
    ...
    assert_eq!(
        format!(
            "{}",
            NaiveTime::from_hms_milli_opt(23, 59, 59, 1_000).unwrap()
        ),
        "23:59:60"
    );
    assert_eq!(
        format!(
            "{}",
            NaiveTime::from_hms_milli_opt(23, 59, 59, 1_001).unwrap()
        ),
        "23:59:60.001"
    );
    assert_eq!(
...
fn test_time_parse_from_str() {
    ...
    assert_eq!(
        NaiveTime::parse_from_str("PM 12:59", "%P %H:%M"),
        Ok(hms(12, 59, 0))
    );
    assert_eq!(
        NaiveTime::parse_from_str("12:59 \n\t PM", "%H:%M \n\t %P"),
        Ok(hms(12, 59, 0))
    );
    assert_eq!(
        NaiveTime::parse_from_str("\t\t12:59\tPM\t", "\t\t%H:%M\t%P\t"),
        Ok(hms(12, 59, 0))
    );
    assert_eq!(
...
fn test_overflowing_offset() {
    ...
    assert_eq!(
        t.overflowing_add_offset(positive_offset),
        (hmsm(9, 6, 7, 890), 0)
    );
    assert_eq!(
        t.overflowing_sub_offset(positive_offset),
        (hmsm(1, 6, 7, 890), 0)
    );
    ...
    assert_eq!(
        t.overflowing_add_offset(positive_offset),
        (hmsm(3, 59, 59, 1_000), 1)
    );
    assert_eq!(
        t.overflowing_sub_offset(positive_offset),
        (hmsm(19, 59, 59, 1_000), 0)
    );
    ...
    let t = hmsm(5, 6, 7, 890);
    assert_eq!(
        t.overflowing_add_offset(negative_offset),
        (hmsm(3, 3, 3, 890), 0)
    );
    assert_eq!(
        t.overflowing_sub_offset(negative_offset),
        (hmsm(7, 9, 11, 890), 0)
    );

    assert_eq!(
        t.overflowing_add_offset(positive_offset).0,
        t + positive_offset
    );
    assert_eq!(
        t.overflowing_sub_offset(positive_offset).0,
        t - positive_offset
    );
}
