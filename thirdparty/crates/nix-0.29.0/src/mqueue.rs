...
pub fn mq_open<P>(name: &P, oflag: MQ_OFlag, mode: Mode, attr: Option<&MqAttr>) -> Result<MqdT>
where
    P: ?Sized + NixPath,
{
...
where
    P: ?Sized + NixPath,
{
    let res = name.with_nix_path(|cstr| unsafe { libc::mq_unlink(cstr.as_ptr()) })?;
...
pub fn mq_send(mqdes: &MqdT, message: &[u8], msq_prio: u32) -> Result<()> {
    let res = unsafe { libc::mq_send(mqdes.0, message.as_ptr().cast(), message.len(), msq_prio) };
    Errno::result(res).map(drop)
}
