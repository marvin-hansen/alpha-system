...
fn test_rfork_and_waitpid() {
    ...
    // Safe: Child only calls `_exit`, which is signal-safe
    match unsafe { rfork(RforkFlags::RFPROC | RforkFlags::RFTHREAD) }.expect("Error: Rfork Failed")
    {
        Child => unsafe { _exit(0) },
        Parent { child } => {
            // assert that child was created and pid > 0
...
fn test_mkfifoat() {
    ...
    let stats = stat::fstatat(Some(dirfd), mkfifoat_name, fcntl::AtFlags::empty()).unwrap();
    let typ = stat::SFlag::from_bits_truncate(stats.st_mode);
    assert_eq!(typ, SFlag::S_IFIFO);
}
...
fn test_mkfifoat_directory_none() {
    ...
    mkfifoat(None, &env::temp_dir(), Mode::S_IRUSR).expect_err("assertion failed");
}
...
fn test_mkfifoat_directory() {
    ...
    mkfifoat(Some(dirfd), mkfifoat_dir, Mode::S_IRUSR).expect_err("assertion failed");
}
...
fn test_getcwd() {
    ...
    for _ in 0..5 {
        ...
        mkdir(inner_tmp_dir.as_path(), Mode::S_IRWXU).expect("assertion failed");
    }
...
fn test_pipe() {
    ...
    let m0 =
        stat::SFlag::from_bits_truncate(stat::fstat(fd0.as_raw_fd()).unwrap().st_mode as mode_t);
    // S_IFIFO means it's a pipe
    assert_eq!(m0, SFlag::S_IFIFO);
    let m1 =
        stat::SFlag::from_bits_truncate(stat::fstat(fd1.as_raw_fd()).unwrap().st_mode as mode_t);
...
fn test_pipe2() {
    ...
    let f0 = FdFlag::from_bits_truncate(fcntl(fd0.as_raw_fd(), FcntlArg::F_GETFD).unwrap());
    assert!(f0.contains(FdFlag::FD_CLOEXEC));
    let f1 = FdFlag::from_bits_truncate(fcntl(fd1.as_raw_fd(), FcntlArg::F_GETFD).unwrap());
...
fn test_linkat_follow_symlink() {
    ...
    assert_eq!(
        stat::SFlag::from_bits_truncate(newfilestat.st_mode as mode_t) & SFlag::S_IFMT,
        SFlag::S_IFREG
    );
...
fn test_access_file_exists() {
    ...
    access(&path, AccessFlags::R_OK | AccessFlags::W_OK).expect("assertion failed");
}
...
fn test_eaccess_file_exists() {
    ...
    let path = tempdir.path().join("does_exist.txt");
    let _file = File::create(path.clone()).unwrap();
    eaccess(&path, AccessFlags::R_OK | AccessFlags::W_OK).expect("assertion failed");
}
