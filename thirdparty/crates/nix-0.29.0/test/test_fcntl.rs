...
#[cfg(all(
    target_os = "linux",
    target_env = "gnu",
    any(target_arch = "x86_64", target_arch = "powerpc", target_arch = "s390x")
))]
...
fn test_openat2_forbidden() {
    ...
    let escape_attempt = tmp.path().parent().unwrap().join("../../../hello.txt");
...
fn test_copy_file_range() {
    ...
    copy_file_range(tmp1.as_fd(), Some(&mut from_offset), tmp2.as_fd(), None, 3).unwrap();
...
mod linux_android {
    ...
    fn test_splice() {
        ...
        let res = splice(tmp, Some(&mut offset), wr, None, 2, SpliceFFlags::empty()).unwrap();
    ...
    fn test_tee() {
        ...
        let res = tee(rd1.try_clone().unwrap(), wr2, 2, SpliceFFlags::empty()).unwrap();
...
fn test_f_get_path() {
    ...
    let res = fcntl(fd, FcntlArg::F_GETPATH(&mut path)).expect("get path failed");
    assert_ne!(res, -1);
    assert_eq!(
...
fn test_f_get_path_nofirmlink() {
    ...
    let fd = tmp.as_raw_fd();
    let mut path = PathBuf::new();
    let res = fcntl(fd, FcntlArg::F_GETPATH_NOFIRMLINK(&mut path)).expect("get path failed");
...
mod test_flock {
    ...
    fn upgrade() {
        ...
        // Attempt to lock second handle
        {
            Flock::lock(file2, FlockArg::LockSharedNonblock).expect("Locking should've succeeded");
        }
