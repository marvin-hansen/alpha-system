...
use nix::{
    ...
    sys::{
        aio::*,
        signal::{sigaction, SaFlags, SigAction, SigHandler, SigSet, SigevNotify, Signal},
...
mod aio_fsync {
    ...
    fn error() {
        ...
        let mut aiof = Box::pin(AioFsync::new(f.as_fd(), mode, 0, SigevNotify::SigevNone));
...
mod aio_read {
    ...
    fn cancel() {
        ...
        f.write_all(INITIAL).unwrap();
        let fd = f.as_fd();
        let mut aior = Box::pin(AioRead::new(fd, 2, &mut rbuf, 0, SigevNotify::SigevNone));
    ...
    fn on_stack() {
        ...
        {
            let fd = f.as_fd();
            let mut aior = AioRead::new(fd, 2, &mut rbuf, 0, SigevNotify::SigevNone);
...
mod aio_readv {
    ...
    fn ok() {
        ...
        {
            let fd = f.as_fd();
            let mut aior = Box::pin(AioReadv::new(fd, 2, &mut rbufs, 0, SigevNotify::SigevNone));
            aior.as_mut().submit().unwrap();
...
mod aio_write {
    ...
    fn cancel() {
        ...
        let f = tempfile().unwrap();
        let mut aiow = Box::pin(AioWrite::new(f.as_fd(), 0, wbuf, 0, SigevNotify::SigevNone));
