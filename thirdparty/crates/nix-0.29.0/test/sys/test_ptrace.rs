...
fn test_ptrace() {
    ...
    assert!(err == Errno::EPERM || err == Errno::EINVAL || err == Errno::ENOSYS);
}
...
fn test_ptrace_syscall() {
    ...
    match unsafe { fork() }.expect("Error: Fork Failed") {
        ...
        Parent { child } => {
            ...
            let get_syscall_id = || ptrace::getregs(child).unwrap().regs[8] as libc::c_long;

            #[cfg(target_arch = "riscv64")]
            let get_syscall_id = || ptrace::getregs(child).unwrap().a7 as libc::c_long;
...
fn test_ptrace_regsets() {
    ...
    match unsafe { fork() }.expect("Error: Fork Failed") {
        ...
        Parent { child } => {
            ...
            let mut regstruct = getregset::<regset::NT_PRSTATUS>(child).unwrap();
            let mut fpregstruct = getregset::<regset::NT_PRFPREG>(child).unwrap();

            #[cfg(target_arch = "x86_64")]
            let (reg, fpreg) = (&mut regstruct.r15, &mut fpregstruct.st_space[5]);
            #[cfg(target_arch = "x86")]
            let (reg, fpreg) = (&mut regstruct.edx, &mut fpregstruct.st_space[5]);
            #[cfg(target_arch = "aarch64")]
            let (reg, fpreg) = (&mut regstruct.regs[16], &mut fpregstruct.vregs[5]);
            #[cfg(target_arch = "riscv64")]
