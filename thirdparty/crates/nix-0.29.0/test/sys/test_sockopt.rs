...
fn test_so_tcp_maxseg() {
    use nix::sys::socket::{accept, bind, connect, getsockname, listen, Backlog, SockaddrIn};
...
fn test_ts_clock_realtime() {
    ...
    setsockopt(&fd, sockopt::TsClock, &SocketTimestamp::SO_TS_REALTIME).unwrap();
    assert_eq!(
        getsockopt(&fd, sockopt::TsClock).unwrap(),
...
fn test_ts_clock_monotonic() {
    ...
    setsockopt(&fd, sockopt::TsClock, &SocketTimestamp::SO_TS_MONOTONIC).unwrap();
    assert_eq!(
        getsockopt(&fd, sockopt::TsClock).unwrap(),
...
fn test_ip_bind_address_no_port() {
    ...
    .unwrap();
    setsockopt(&fd, sockopt::IpBindAddressNoPort, &true)
        .expect("setting IP_BIND_ADDRESS_NO_PORT on an inet stream socket should succeed");
    assert!(getsockopt(&fd, sockopt::IpBindAddressNoPort)
        .expect("getting IP_BIND_ADDRESS_NO_PORT on an inet stream socket should succeed",));
    setsockopt(&fd, sockopt::IpBindAddressNoPort, &false)
        .expect("unsetting IP_BIND_ADDRESS_NO_PORT on an inet stream socket should succeed");
    assert!(!getsockopt(&fd, sockopt::IpBindAddressNoPort)
        .expect("getting IP_BIND_ADDRESS_NO_PORT on an inet stream socket should succeed",));
}
...
fn test_tcp_fast_open_connect() {
    ...
    .unwrap();
    setsockopt(&fd, sockopt::TcpFastOpenConnect, &true)
        .expect("setting TCP_FASTOPEN_CONNECT on an inet stream socket should succeed");
    assert!(getsockopt(&fd, sockopt::TcpFastOpenConnect)
        .expect("getting TCP_FASTOPEN_CONNECT on an inet stream socket should succeed",));
    setsockopt(&fd, sockopt::TcpFastOpenConnect, &false)
        .expect("unsetting TCP_FASTOPEN_CONNECT on an inet stream socket should succeed");
    assert!(!getsockopt(&fd, sockopt::TcpFastOpenConnect)
        .expect("getting TCP_FASTOPEN_CONNECT on an inet stream socket should succeed",));
}
...
fn is_socket_type_dgram() {
    use nix::sys::socket::{getsockopt, sockopt, AddressFamily, SockFlag, SockType};
...
fn can_get_listen_on_tcp_socket() {
    use nix::sys::socket::{
        getsockopt, listen, socket, sockopt, AddressFamily, Backlog, SockFlag, SockType,
    };
...
fn test_utun_ifname() {
    ...
    let unit = 123;
    let addr =
        SysControlAddr::from_name(fd.as_raw_fd(), "com.apple.net.utun_control", unit).unwrap();
