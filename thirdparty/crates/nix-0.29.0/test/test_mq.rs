...
use nix::errno::Errno;
use nix::mqueue::{mq_attr_member_t, mq_close, mq_open, mq_receive, mq_send, mq_timedreceive};
...
fn test_mq_timedreceive() {
    ...
    let mut buf = [0u8; 32];
    let mut prio = 0u32;
    let abstime = clock_gettime(ClockId::CLOCK_REALTIME).unwrap() + TimeSpec::seconds(1);
