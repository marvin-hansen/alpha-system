    ...
    ($self:ident) => {{
        ...
        while index > 0
            && $self
                ...
                .slc
                .get(index - 1)
                .map_or(false, |&x| $self.is_digit_separator(x))
        {
            index -= 1;
        }
        ...
        index == 0
            || !$self
                ...
                .slc
                .get(index - 1)
                .map_or(false, |&x| $self.is_digit(x))
    }};
}
...
skip_iterator!(
    ...
    "Iterator that skips over digit separators in the integer."
);
