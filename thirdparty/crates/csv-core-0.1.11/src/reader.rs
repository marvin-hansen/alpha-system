...
impl DfaClasses {
    ...
    /// the main DFA loop.
    #[inline(always)]
    fn scan_and_copy(&self, input: &[u8], nin: &mut usize, output: &mut [u8], nout: &mut usize) {
        while *nin < input.len() && *nout < output.len() && self.classes[input[*nin] as usize] == 0
        {
            output[*nout] = input[*nin];
            *nin += 1;
            *nout += 1;
...
mod tests {
    ...
    fn parse_by_record(rdr: &mut Reader, data: &str) -> Csv {
        ...
        loop {
            let (res, nin, nout, nend) =
                rdr.read_record(data, &mut record[outpos..], &mut ends[endpos..]);
            data = &data[nin..];
            outpos += nout;
            endpos += nend;
    ...
    fn stream_empty_output() {
        ...
        assert_read!(rdr, b("foo,bar"), out, 4, 3, Field { record_end: false });
    ...
    macro_rules! assert_read_record {
        (
            ...
            let (res, nin, nout, nend) = $rdr.read_record($input, $output, $ends);
