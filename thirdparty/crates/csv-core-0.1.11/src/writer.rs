...
impl Writer {
    ...
    pub fn field(&mut self, input: &[u8], mut output: &mut [u8]) -> (WriteResult, usize, usize) {
    ...
    pub fn delimiter(&mut self, mut output: &mut [u8]) -> (WriteResult, usize) {
        let mut nout = 0;
        if self.state.quoting {
            let (res, o) = self.write(&[self.quote], output);
            if o == 0 {
    ...
    pub fn terminator(&mut self, mut output: &mut [u8]) -> (WriteResult, usize) {
        let mut nout = 0;
...
pub fn quote(
    ...
    loop {
        match memchr(quote, input) {
            ...
            Some(next_quote) => {
                let (res, i, o) = write_optimistic(&input[..next_quote], output);
                input = &input[i..];
                output = &mut moving(output)[o..];
                nin += i;
                nout += o;
...
fn write_optimistic(input: &[u8], output: &mut [u8]) -> (WriteResult, usize, usize) {
...
mod tests {
    ...
    fn writer_two_fields_quote() {
        ...
        assert_field!(wtr, b("a,bc"), &mut out[n..], 4, 5, InputEmpty, "\"a,bc");
        n += 5;
        assert_write!(wtr, delimiter, &mut out[n..], 2, InputEmpty, "\",");
        n += 2;
    ...
    fn writer_two_fields_two_records_quote() {
        ...
        assert_write!(wtr, delimiter, &mut out[n..], 2, InputEmpty, "\",");
        n += 2;
        assert_field!(wtr, b("quux,"), &mut out[n..], 5, 6, InputEmpty, "\"quux,");
        n += 6;
    ...
    fn quote_misc() {
        ...
        assert_quote!(inp, out, 21, 25, InputEmpty, r#"foo ""bar"" baz ""quux""?"#);
    }
    ...
    fn comment_char_is_automatically_quoted() {
        ...
        assert_field!(wtr, b("# abc"), &mut out[..], 5, 6, InputEmpty, "\"# abc");
