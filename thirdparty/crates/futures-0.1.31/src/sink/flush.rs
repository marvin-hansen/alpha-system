...
impl<S: Sink> Flush<S> {
    ...
    pub fn get_ref(&self) -> &S {
        self.sink
            .as_ref()
            .expect("Attempted `Flush::get_ref` after the flush completed")
    }
    ...
    pub fn get_mut(&mut self) -> &mut S {
        self.sink
            .as_mut()
            .expect("Attempted `Flush::get_mut` after the flush completed")
    }

    /// Consume the `Flush` and return the inner sink.
    pub fn into_inner(self) -> S {
        self.sink
            .expect("Attempted `Flush::into_inner` after the flush completed")
    }
}
...
impl<S: Sink> Future for Flush<S> {
    ...
    fn poll(&mut self) -> Poll<S, S::SinkError> {
        let mut sink = self
            .sink
            .take()
            .expect("Attempted to poll Flush after it completed");
