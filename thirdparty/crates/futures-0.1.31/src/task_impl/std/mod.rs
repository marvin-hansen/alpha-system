...
use super::core;
use super::{spawn, BorrowedTask, Notify, NotifyHandle, Spawn, UnsafeNotify};
use {Async, AsyncSink, Future, Poll, Sink, StartSend, Stream};
...
impl TaskUnpark {
    ...
    pub fn will_notify(&self, unpark: &BorrowedUnpark) -> bool {
        match (unpark, self) {
            ...
            (&BorrowedUnpark::New(ref new1), &TaskUnpark::New(ref new2)) => new2.will_notify(new1),
            _ => false,
        }
    }
}
...
impl<F: Future> Spawn<F> {
    ...
    pub fn wait_future(&mut self) -> Result<F::Item, F::Error> {
        ThreadNotify::with_current(|notify| loop {
...
impl<S: Sink> Spawn<S> {
    ...
    #[allow(deprecated)]
    #[doc(hidden)]
    pub fn poll_flush(&mut self, unpark: &Arc<Unpark>) -> Poll<(), S::SinkError> {
    ...
    pub fn wait_send(&mut self, mut value: S::SinkItem) -> Result<(), S::SinkError> {
        ThreadNotify::with_current(|notify| loop {
        ...
        unsafe {
            ...
            loop {
                match spawn.poll_future_notify(&inner, 0) {
                    Ok(Async::NotReady) => {}
                    Ok(Async::Ready(())) | Err(()) => return inner.mutex.complete(),
                }
                let run = Run {
                    spawn: spawn,
                    inner: inner.clone(),
...
impl fmt::Debug for Run {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("Run").field("contents", &"...").finish()
    }
}
...
#[deprecated(
    since = "0.1.18",
    note = "recommended to use `FuturesUnordered` instead"
)]
...
impl<T: Notify + 'static> Notify for ArcWrapped<T> {
    fn notify(&self, id: usize) {
        unsafe {
            ...
            T::notify(&*(&me as *const *const ArcWrapped<T> as *const Arc<T>), id)
        }
    }
    ...
    fn clone_id(&self, id: usize) -> usize {
        unsafe {
            let me: *const ArcWrapped<T> = self;
            T::clone_id(&*(&me as *const *const ArcWrapped<T> as *const Arc<T>), id)
        }
    }
    ...
    fn drop_id(&self, id: usize) {
        unsafe {
            let me: *const ArcWrapped<T> = self;
            T::drop_id(&*(&me as *const *const ArcWrapped<T> as *const Arc<T>), id)
        }
    }
}
