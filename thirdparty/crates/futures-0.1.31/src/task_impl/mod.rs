...
use {Future, Poll, Sink, StartSend, Stream};
...
impl Task {
    ...
    pub fn will_notify_current(&self) -> bool {
        with(|current| {
            self.unpark.will_notify(&current.unpark) && self.events.will_notify(&current.events)
        })
    }
}
...
impl<T: ?Sized> Spawn<T> {
    ...
    /// pointer.
    pub fn poll_future_notify<N>(&mut self, notify: &N, id: usize) -> Poll<T::Item, T::Error>
    where
        N: Clone + Into<NotifyHandle>,
        T: Future,
    {
...
impl<T: fmt::Debug + ?Sized> fmt::Debug for Spawn<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("Spawn").field("obj", &&self.obj).finish()
    }
}
