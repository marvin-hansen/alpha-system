...
impl<S: Debug> Debug for Concat2<S>
where
    S: Stream,
    S::Item: Debug,
{
...
#[allow(deprecated)]
impl<S: Debug> Debug for Concat<S>
where
    S: Stream,
    S::Item: Debug,
{
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        loop {
            match self.stream.poll() {
                ...
                Ok(Async::Ready(None)) => match mem::replace(&mut self.extend, Inner::Done) {
