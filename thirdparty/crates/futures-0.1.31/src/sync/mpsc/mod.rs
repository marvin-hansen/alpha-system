...
use sink::SendAll;
use sync::mpsc::queue::{PopResult, Queue};
use sync::oneshot;
use task::{self, Task};
...
impl<T> Sender<T> {
    ...
    pub fn try_send(&mut self, msg: T) -> Result<(), TrySendError<T>> {
        ...
        self.do_send(Some(msg), false)
            .map_err(|SendError(v)| TrySendError {
...
pub struct Execute<S: Stream> {
    inner: SendAll<
        Sender<Result<S::Item, S::Error>>,
        Results<S, SendError<Result<S::Item, S::Error>>>,
    >,
...
impl<I, E> fmt::Debug for SpawnHandle<I, E> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("SpawnHandle").finish()
    }
}
