...
fn unbounded_1_tx(b: &mut Bencher) {
    b.iter(|| {
        ...
        for i in 0..1000 {
            ...
            assert_eq!(
                Ok(Async::NotReady),
                rx.poll_stream_notify(&notify_noop(), 1)
            );
...
fn bounded_1_tx(b: &mut Bencher) {
    b.iter(|| {
        ...
        let mut tx = executor::spawn(TestSender { tx: tx, last: 0 });

        let mut rx = executor::spawn(rx);

        for i in 0..1000 {
            assert_eq!(
                Ok(Async::Ready(Some(i + 1))),
                tx.poll_stream_notify(&notify_noop(), 1)
            );
            assert_eq!(
                Ok(Async::NotReady),
                tx.poll_stream_notify(&notify_noop(), 1)
    ...
    b.iter(|| {
        ...
        for i in 0..10 {
            for j in 0..tx.len() {
                ...
                assert_eq!(
                    Ok(Async::Ready(Some(i + 1))),
                    tx[j].poll_stream_notify(&notify_noop(), 1)
                );
                // Then block
                assert_eq!(
                    Ok(Async::NotReady),
