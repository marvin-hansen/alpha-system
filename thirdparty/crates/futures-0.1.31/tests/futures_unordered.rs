...
fn works_2() {
    ...
    b_tx.send(33).unwrap();
    assert!(spawn
        .poll_stream_notify(&support::notify_noop(), 0)
        .unwrap()
        .is_ready());
    c_tx.send(33).unwrap();
    assert!(spawn
...
fn from_iterator() {
    ...
    let stream = vec![ok::<u32, ()>(1), ok::<u32, ()>(2), ok::<u32, ()>(3)]
        .into_iter()
...
fn finished_future_ok() {
    ...
    let stream = futures_unordered(vec![
        ...
        Box::new(
            b_rx.select(c_rx)
                .then(|res| Ok(Box::new(res) as Box<Any + Send>)),
        ),
    ]);
...
fn iter_mut_cancel() {
    ...
    let mut spawn = futures::executor::spawn(stream);
    assert_eq!(
        Some(Err(futures::sync::oneshot::Canceled)),
        spawn.wait_stream()
    );
    assert_eq!(
        Some(Err(futures::sync::oneshot::Canceled)),
