...
fn and_then1() {
    ...
    let tx2 = tx.clone();
    let p1 = ok::<_, i32>("a").then(move |t| {
        tx2.send("first").unwrap();
        t
    });
    let tx2 = tx.clone();
    let p2 = ok("b").then(move |t| {
...
fn and_then2() {
    ...
    let tx2 = tx.clone();
    let p1 = err::<i32, _>(2).then(move |t| {
        tx2.send("first").unwrap();
        t
    });
    let tx2 = tx.clone();
...
fn or_else1() {
    ...
    let (tx, rx) = mpsc::channel();
    let tx2 = tx.clone();
    let p1 = p1.map_err(move |i| {
        tx2.send(2).unwrap();
        i
    });
    let tx2 = tx.clone();
    let p2 = p2.map(move |i| {
        tx2.send(i).unwrap();
        i
    });
...
fn select2() {
    ...
    let (tx, rx) = mpsc::channel();
    p1.select(p2)
        .map_err(move |v| tx.send((1, v.1)).unwrap())
        .forget();
    assert!(rx.try_recv().is_err());
    drop(c1);
    let (v, p2) = rx.recv().unwrap();
