...
fn and_then() {
    ...
    assert_done(
        || list().and_then(|a| err::<i32, u32>(a as u32)).collect(),
        Err(1),
    );
}
...
fn wait() {
    assert_eq!(
        list().wait().collect::<Result<Vec<_>, _>>(),
        Ok(vec![1, 2, 3])
    );
}
...
fn forward() {
    ...
    assert_done(
        move || iter_ok(vec![4, 5]).forward(v).map(|(_, s)| s),
...
fn inspect_err() {
    let mut seen = vec![];
    assert_done(
        || err_list().inspect_err(|&a| seen.push(a)).collect(),
        Err(3),
    );
    assert_eq!(seen, [3]);
}
