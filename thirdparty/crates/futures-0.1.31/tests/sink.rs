...
fn send() {
    ...
    assert_done(move || v.send(2), Ok(vec![0, 1, 2]));
}
...
fn with_as_map() {
    let sink = Vec::new().with(|item| -> Result<i32, ()> { Ok(item * 2) });
...
fn with_flat_map() {
    let sink = Vec::new().with_flat_map(|item| stream::iter_ok(vec![item; item]));
    let sink = sink.send(0).wait().unwrap();
    let sink = sink.send(1).wait().unwrap();
    let sink = sink.send(2).wait().unwrap();
