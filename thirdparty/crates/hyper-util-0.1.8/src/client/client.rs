...
where
    M: MakeService<hyper::Uri, Request<()>, Response = Response<()>, Error = E, MakeError = E>,
...
impl<M, /*ReqBody, ResBody,*/ E> Client<M> where
    M: MakeService<hyper::Uri, Request<()>, Response = Response<()>, Error = E, MakeError = E>
    //M: Service<hyper::Uri, Error = E>,
    //M::Response: Service<Request<ReqBody>, Response = Response<ResBody>>,
{
    ...
    C::_Svc: Unpin + Send + 'static,
{
    async fn connection_for(
        &self,
        target: PoolKey,
    ) -> Result<pool::Pooled<PoolableSvc<C::_Svc>, PoolKey>, ()> {
        todo!()
    }
}
