...
use regex::Regex;
use syn::{
    parse_macro_input, spanned::Spanned, DeriveInput, Expr, ExprLit, ExprPath, Fields,
    FieldsUnnamed, Lifetime, Lit, Meta, Type,
};
...
fn impl_token_parse(input: &DeriveInput) -> proc_macro2::TokenStream {
    if input.generics.params.len() > 1
        || !matches!(
            ...
            None | Some(syn::GenericParam::Lifetime(_))
        )
    {
    ...
    let original_lifetime_param =
        if let Some(syn::GenericParam::Lifetime(lifetime)) = input.generics.params.first() {
    ...
    for variant in &items.variants {
        ...
        if parse_data.parse_fn.is_some()
            && (!parse_data.literals.is_empty()
                || !parse_data.simple_regexes.is_empty()
                || !parse_data.regexes.is_empty())
        {
            return quote_spanned! {
                input.span() =>
        ...
        let has_anything = parse_data.parse_fn.is_some()
            || !parse_data.literals.is_empty()
            || !parse_data.simple_regexes.is_empty()
            || !parse_data.regexes.is_empty();
                    ...
                    Type::Reference(ty) => {
                        ...
                        if let Type::Path(path) = &*ty.elem {
                            if path.qself.is_some()
                                ...
                                || path.path.segments.first().unwrap().ident != "str"
                            {
                                return quote_spanned! {
                                    unnamed.span() =>
    ...
    let mut simple_regex_ident_conflicts: BTreeMap<(Ident, String), Vec<(Ident, String)>> =
        BTreeMap::new();
    let mut regex_ident_conflicts: BTreeMap<(Ident, String), Vec<(Ident, String)>> =
        BTreeMap::new();
    ...
    if let Err(e) = gen::simple_regex::gen_simple_regex(
        &tokens_to_parse[..],
        &simple_regexes,
        &simple_regex_ident_conflicts,
        &input.ident,
    ...
    if let Err(e) = gen::full_regex::gen_full_regex(
        &tokens_to_parse[..],
        &regex_ident_conflicts,
        &input.ident,
        &mut parse_fns,
    ...
    let reinput = {
        ...
        for variant in &items.variants {
            let attrs = flatten(variant.attrs.iter().filter(|a| {
                a.path().segments.len() != 1 || a.path().segments.first().unwrap().ident != "token"
            }));
    ...
    if let Some(lit_table_index) = tokens_to_parse.iter().position(|x| !x.literals.is_empty()) {
        parse_fns
            .entry(lit_table_index)
            .or_default()
            .push(lit_table_parse);
    }
