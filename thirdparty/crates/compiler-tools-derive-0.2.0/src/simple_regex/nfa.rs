...
impl TransitionEvent {
    ...
    pub fn completely_shadows(&self, other: &TransitionEvent) -> bool {
        match (self, other) {
            (TransitionEvent::Epsilon, _)
            | (_, TransitionEvent::Epsilon)
            | (_, TransitionEvent::End) => false,
            ...
            match atom.repeat {
                Repeat::Once => {
                    for event in events {
                        self_
                            .transitions
                            .insert(current_state, vec![(event, current_state + 1)]);
                        current_state += 1;
                    }
                }
                Repeat::ZeroOrOnce => {
                    ...
                    self_.transitions.insert(
                        ...
                        vec![
                            ...
                            (
                                ...
                                current_state + events.len() as u32 + 1,
                            ),
                        ],
                    );
                    current_state += 1;
                ...
                Repeat::OnceOrMore => {
                    ...
                    for (i, event) in events.into_iter().enumerate() {
                        ...
                        self_
                            .transitions
                            .insert(current_state, vec![(event, next_state)]);
                        current_state += 1;
                    }
                }
                Repeat::ZeroOrMore => {
