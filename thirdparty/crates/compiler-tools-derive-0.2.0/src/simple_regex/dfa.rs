...
impl Dfa {
    pub fn build(nfa: &Nfa) -> Self {
        ...
        while let Some((state, shadows)) = state_set.pop() {
            ...
            let mut epsilon_closure = epsilon_closure
                .into_iter()
                .enumerate()
                .collect::<HashMap<_, _>>();
            ...
            let mut original_shadowed_closures: HashMap<u32, Vec<(TransitionEvent, u32)>> =
                shadowed_closures
                    .iter()
                    .map(|(shadowed, shadows)| {
                        (
                            ...
                            shadows
                                .into_iter()
                                .map(|shadow| epsilon_closure.get(shadow).unwrap().clone())
                                .collect(),
                        )
                    })
                    .collect();
            ...
            while emitted_closures.len() < total {
                for i in 0..total {
                    ...
                    if !shadowed_closures.contains_key(&i)
                        || shadowed_closures.get(&i).unwrap().is_empty()
                    {
