use {
    ...
    regex_test::{
        CompiledRegex, Match, MatchKind, RegexTest, SearchKind, Span, TestResult, TestRunner,
    },
};
...
fn prefilter() -> Result<()> {
    let my_compiler = |test: &RegexTest, regexes: &[String]| {
        ...
        let pre = Prefilter::from_hirs_prefix(regex_automata::MatchKind::LeftmostFirst, &hirs);
    ...
    runner
        .test_iter(suite()?.iter(), move |test, regexes| {
            ...
            builder.configure(BoundedBacktracker::config().visited_capacity(
                backtrack::min_visited_capacity(&nfa, &Input::new(test.haystack())),
            ));
...
fn run_test(re: &BoundedBacktracker, cache: &mut backtrack::Cache, test: &RegexTest) -> TestResult {
    let input = create_input(test);
    match test.additional_name() {
        "is_match" => match test.search_kind() {
            SearchKind::Earliest | SearchKind::Overlapping => TestResult::skip(),
            SearchKind::Leftmost => {
...
fn configure_backtrack_builder(test: &RegexTest, builder: &mut backtrack::Builder) -> bool {
