use {
    anyhow::Result,
    regex_automata::{
        dfa::{self, dense, regex::Regex, sparse, Automaton, OverlappingState, StartKind},
        nfa::thompson,
...
fn serialization_unminimized_default() -> Result<()> {
    ...
    let my_compiler = |builder| {
        compiler(builder, |builder, _, re| {
            ...
            Ok(CompiledRegex::compiled(move |test| -> TestResult {
                let fwd: dense::DFA<&[u32]> = dense::DFA::from_bytes(&fwd_bytes).unwrap().0;
                let rev: dense::DFA<&[u32]> = dense::DFA::from_bytes(&rev_bytes).unwrap().0;
                let re = builder.build_from_dfas(fwd, rev);
...
fn sparse_serialization_unminimized_default() -> Result<()> {
    ...
    let my_compiler = |builder| {
        compiler(builder, |builder, _, re| {
            ...
            Ok(CompiledRegex::compiled(move |test| -> TestResult {
                let fwd: sparse::DFA<&[u8]> = sparse::DFA::from_bytes(&fwd_bytes).unwrap().0;
                let rev: sparse::DFA<&[u8]> = sparse::DFA::from_bytes(&rev_bytes).unwrap().0;
                let re = builder.build_from_dfas(fwd, rev);
                run_test(&re, test)
            }))
        })
    ...
    match test.additional_name() {
        ...
        "find" => match test.search_kind() {
            SearchKind::Earliest | SearchKind::Leftmost => {
                let input = input.earliest(test.search_kind() == SearchKind::Earliest);
                TestResult::matches(
                    re.find_iter(input)
            ...
            SearchKind::Overlapping => try_search_overlapping(re, &input).unwrap(),
        },
...
fn configure_regex_builder(test: &RegexTest, builder: &mut dfa::regex::Builder) -> bool {
...
fn try_search_overlapping<A: Automaton>(re: &Regex<A>, input: &Input<'_>) -> Result<TestResult> {
    let mut matches = vec![];
