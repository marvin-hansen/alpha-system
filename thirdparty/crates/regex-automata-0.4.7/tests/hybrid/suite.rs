...
fn run_test(re: &Regex, cache: &mut regex::Cache, test: &RegexTest) -> TestResult {
    let input = create_input(test);
    match test.additional_name() {
        "is_match" => TestResult::matched(re.is_match(cache, input.earliest(true))),
        "find" => match test.search_kind() {
            ...
            SearchKind::Overlapping => try_search_overlapping(re, cache, &input).unwrap(),
        },
...
fn configure_regex_builder(test: &RegexTest, builder: &mut regex::Builder) -> bool {
...
fn try_search_overlapping(
    ...
    while let Some(end) = {
        ...
        while let Some(start) = {
            rev_dfa.try_search_overlapping_rev(rev_cache, &revsearch, &mut rev_state)?;
            rev_state.get_match()
        } {
            let span = Span {
                start: start.offset(),
