...
fn make_slots<T, const LEN: usize>() -> ([MaybeUninit<T>; LEN], [MaybeUninit<T>; LEN])
where
    T: BitStore,
{
    (
...
pub fn benchmarks(crit: &mut Criterion) {
    fn steps() -> impl Iterator<Item = (impl Fn(&'static str) -> BenchmarkId, usize, Throughput)> {
        SCALARS.iter().map(|&n| {
            (
    ...
    let (src_words, mut dst_words) = make_slots::<usize, { elts::<usize>(MAX_BITS) }>();
    let (src_bytes, mut dst_bytes) = make_slots::<u8, { elts::<u8>(MAX_BITS) }>();
    ...
    for (id, bits, thrpt) in steps() {
        ...
        let (src_words, dst_words) = (&src_words[..words], &mut dst_words[..words]);
        let (src_bytes, dst_bytes) = (&src_bytes[..bytes], &mut dst_bytes[..bytes]);
    ...
    for (id, bits, thrpt) in steps() {
        ...
        group.bench_function(id("bitwise_words"), |b| {
            ...
            b.iter(|| unsafe {
                for (from, to) in src.as_bitptr_range().zip(dst.as_mut_bitptr_range()) {
                    to.write(from.read());
                }
            })
        });
