...
use core::{
    ...
    fmt::Debug,
    hash::Hash,
    ops::{BitAnd, BitOr, BitXor, Index, IndexMut, Range},
};
...
mod format {
    ...
    use super::{BitArray, Lsb0};

    #[test]
    fn render() {
        let render = format!("{:?}", BitArray::<u8, Lsb0>::ZERO);
        assert!(render.starts_with(&format!("BitArray<u8, {}>", any::type_name::<Lsb0>(),)));
        assert!(render.ends_with("[0, 0, 0, 0, 0, 0, 0, 0]"));

        assert_eq!(
            format!(
                "{:?}",
                BitArray::<u8, Lsb0>::try_from(bits![u8, Lsb0; 0, 1]).unwrap_err(),
            ),
        ...
        assert_eq!(
            format!(
                "{:?}",
                BitArray::<u8, Lsb0>::try_from(&bits![u8, Lsb0; 0; 9][1..]).unwrap_err(),
            ),
