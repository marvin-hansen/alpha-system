    ...
    R: BitRegister,
{
    ...
    pub fn range_from(
        ...
        (from.idx..=Self::MAX.end).map(|tail| unsafe { BitEnd::new_unchecked(tail) })
    }
    ...
    R: BitRegister,
{
    ...
    pub(crate) fn range_all(
        ...
        BitIdx::<R>::range_all().map(|idx| unsafe { Self::new_unchecked(idx.into_inner()) })
    }
}
...
mod tests {
    ...
    fn render() {
        ...
        #[cfg(target_pointer_width = "64")]
        {
            assert_eq!(format!("{:?}", BitIdx::<u64>::MAX), "BitIdx<u64>(111111)",);
            assert_eq!(
                format!("{:?}", BitIdx::<u64>::new(64).unwrap_err()),
                "BitIdxError<u64>(64)",
            );
            assert_eq!(format!("{:?}", BitEnd::<u64>::MAX), "BitEnd<u64>(1000000)",);
            assert_eq!(format!("{:?}", BitPos::<u64>::MAX), "BitPos<u64>(111111)",);
            assert_eq!(
