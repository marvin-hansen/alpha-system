...
use super::{
    BitSlice, Chunks, ChunksExact, ChunksExactMut, ChunksMut, Iter, IterMut, RChunks, RChunksExact,
    RChunksExactMut, RChunksMut, RSplit, RSplitMut, RSplitN, RSplitNMut, Split, SplitInclusive,
    SplitInclusiveMut, SplitMut, SplitN, SplitNMut, Windows,
};
#[cfg(feature = "alloc")]
    ...
    O: BitOrder,
{
    ...
    #[inline]
    pub fn chunks_exact_mut(&mut self, chunk_size: usize) -> ChunksExactMut<T, O> {
    ...
    #[inline]
    pub fn rchunks_exact_mut(&mut self, chunk_size: usize) -> RChunksExactMut<T, O> {
    ...
    /// [`.split_mut()`]: Self::split_mut
    #[inline]
    pub fn split_inclusive_mut<F>(&mut self, pred: F) -> SplitInclusiveMut<T, O, F>
    where
        F: FnMut(usize, &bool) -> bool,
    {
    ...
    where
        ...
        O2: BitOrder,
    {
        self.len() >= other.len() && self.windows(other.len()).any(|window| window == other)
    }
    ...
    O: BitOrder,
{
    ...
    fn index(self, bits: &'a BitSlice<T, O>) -> Self::Immut {
        self.get(bits)
            .unwrap_or_else(|| panic!("index {} out of bounds: {}", self, bits.len()))
    }
...
macro_rules! range_impl {
    ($r:ty { check $check:expr; select $select:expr; }) => {
            ...
            T: BitStore,
        {
            ...
            #[inline]
            #[allow(clippy::blocks_in_if_conditions, clippy::redundant_closure_call)]
            ...
            fn index(self, bits: Self::Immut) -> Self::Immut {
                ...
                let l = bits.len();
                self.get(bits)
                    .unwrap_or_else(|| panic!("range {:?} out of bounds: {}", r, l))
            }
