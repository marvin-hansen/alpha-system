...
fn copying() {
    ...
    a.view_bits_mut::<Lsb0>()[4..92].copy_from_bitslice(&b.view_bits::<Lsb0>()[4..92]);
    assert_eq!(
        [a[0] & 0xFF_FF_FF_F0, a[1], a[2] & 0x0F_FF_FF_FF],
        [b[0] & 0xFF_FF_FF_F0, b[1], b[2] & 0x0F_FF_FF_FF],
    );

    let mut c = random::<u32>();
    let d = random::<u32>();
    c.view_bits_mut::<Msb0>()[4..28].copy_from_bitslice(&d.view_bits::<Msb0>()[4..28]);
    assert_eq!(c & 0x0F_FF_FF_F0, d & 0x0F_FF_FF_F0);

    let mut e = 0x01_23_45_67u32;
    let f = 0x89_AB_CD_EFu32;
    e.view_bits_mut::<HiLo>()[..28].copy_from_bitslice(&f.view_bits::<HiLo>()[4..]);
...
fn bit_counting() {
    ...
    assert!([!0u8; 3].view_bits::<Lsb0>()[1..23].first_zero().is_none());
    assert!([!0u8; 1].view_bits::<Msb0>()[1..7].first_zero().is_none());
    assert!([!0u8; 3].view_bits::<Msb0>()[1..23].first_zero().is_none());
...
fn cooking() {
    ...
    unsafe {
        assert_eq!(
            slice::from_raw_parts_unchecked(BitPtr::try_from(ref_ptr).unwrap(), len).as_bitspan(),
            data.view_bits::<Lsb0>().as_bitspan(),
        );
        assert_eq!(
            slice::from_raw_parts_unchecked_mut(BitPtr::try_from(mut_ptr).unwrap(), len)
                .as_bitspan(),
