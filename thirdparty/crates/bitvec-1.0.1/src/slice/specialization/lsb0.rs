...
use funty::Integral;
use wyz::{bidi::BidiIterator, range::RangeExt};

use super::{has_one, has_zero, WORD_BITS};
use crate::{
    domain::Domain, field::BitField, mem::bits_of, order::Lsb0, slice::BitSlice, store::BitStore,
};
    ...
    T: BitStore,
{
    ...
    pub(crate) fn sp_copy_from_bitslice(&mut self, src: &Self) {
        ...
        for (to, from) in
            unsafe { self.chunks_mut(WORD_BITS).remove_alias() }.zip(src.chunks(WORD_BITS))
        {
    ...
    pub(crate) unsafe fn sp_copy_within_unchecked(
        ...
        let to = this
            .get_unchecked(dest)
            .chunks(WORD_BITS)
            .map(|bits| bits as *const BitSlice<T::Access, Lsb0> as *mut BitSlice<T::Access, Lsb0>);
        ...
        match self.domain() {
            Domain::Enclave(elem) => {
                ...
                if has_one(val, elem.mask().into_inner()) {
                    accum += val.trailing_zeros() as usize - elem.head().into_inner() as usize;
                    return Some(accum);
                }
                None
            }
    ...
    pub(crate) fn sp_last_one(&self) -> Option<usize> {
        ...
        match self.domain() {
            Domain::Enclave(elem) => {
                let val = elem.load_value();
                let dead_bits = bits_of::<T::Mem>() - elem.tail().into_inner() as usize;
                if has_one(val, elem.mask().into_inner()) {
    ...
    pub(crate) fn sp_first_zero(&self) -> Option<usize> {
        ...
        match self.domain() {
            Domain::Enclave(elem) => {
                ...
                accum += val.trailing_ones() as usize - elem.head().into_inner() as usize;
