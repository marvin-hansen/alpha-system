...
use core::{
    cmp,
    convert::TryFrom,
    fmt::{self, Binary, Debug, Display, Formatter, LowerHex, Octal, Pointer, UpperHex},
    hash::{Hash, Hasher},
    str,
};
    ...
    O2: BitOrder,
{
    ...
    fn eq(&self, rhs: &BitSlice<T2, O2>) -> bool {
        if let (Some(this), Some(that)) = (self.coerce::<T1, Lsb0>(), rhs.coerce::<T1, Lsb0>()) {
            this.sp_eq(that)
    ...
    O: BitOrder,
{
    ...
    fn try_from(slice: &'a mut [T]) -> Result<Self, Self::Error> {
        ...
        BitSlice::try_from_slice_mut(slice).map_err(|_| unsafe { &mut *slice_ptr })
    }
}
    ...
    O: BitOrder,
{
    ...
    match val {
        v @ 0..=9 => b'0' + v,
        v @ 10..=35 => alpha - 10 + v,
        _ => unreachable!("bit-slices wider than five bits cannot be rendered to ASCII b36"),
    }
}
...
where
    ...
    O: BitOrder,
{
    for (chunk, slot) in bits.rchunks(radix).rev().zip(into.iter_mut().skip(skip)) {
