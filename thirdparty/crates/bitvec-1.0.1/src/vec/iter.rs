    ...
    T: 'a + BitStore,
{
    ...
    fn fill(&mut self, iter: &mut impl Iterator<Item = bool>) -> FillStatus {
        ...
        let mut len = bv.len();
        let span = unsafe { bv.as_mut_bitptr().add(len).range(self.tail.start - len) };
    ...
    unsafe fn move_tail(&mut self, additional: usize) {
        ...
        let orig_tail = mem::replace(&mut self.tail, new_tail_start..new_tail_start + tail_len);
        let len = bv.len();
    ...
    I: Iterator<Item = bool>,
{
    ...
    fn drop(&mut self) {
         ...
         */
        let mut collected = self.splice.by_ref().collect::<BitVec<T, O>>().into_iter();
