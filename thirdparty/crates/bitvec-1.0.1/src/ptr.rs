...
use crate::{devel as dvl, order::BitOrder, slice::BitSlice, store::BitStore};
...
pub use wyz::comu::{Const, Mut, Mutability};

pub(crate) use self::{addr::AddressExt, span::BitSpan};
pub use self::{
    addr::{check_alignment, MisalignError},
    proxy::BitRef,
    range::BitPtrRange,
    single::{BitPtr, BitPtrError},
    span::BitSpanError,
};
...
#[inline]
pub fn eq<T1, T2, O>(this: BitPtr<Const, T1, O>, that: BitPtr<Const, T2, O>) -> bool
where
    T1: BitStore,
    T2: BitStore,
    O: BitOrder,
...
pub fn slice_from_raw_parts<T, O>(ptr: BitPtr<Const, T, O>, len: usize) -> *const BitSlice<T, O>
where
    T: BitStore,
    O: BitOrder,
{
...
pub fn slice_from_raw_parts_mut<T, O>(ptr: BitPtr<Mut, T, O>, len: usize) -> *mut BitSlice<T, O>
where
    T: BitStore,
    O: BitOrder,
{
...
pub unsafe fn swap<T1, T2, O1, O2>(one: BitPtr<Mut, T1, O1>, two: BitPtr<Mut, T2, O2>)
where
    T1: BitStore,
    T2: BitStore,
    O1: BitOrder,
...
pub unsafe fn write_bytes<T, O>(dst: BitPtr<Mut, T, O>, value: bool, count: usize)
where
    T: BitStore,
    O: BitOrder,
{
...
pub fn bitslice_from_raw_parts<T, O>(ptr: BitPtr<Const, T, O>, len: usize) -> *const BitSlice<T, O>
where
    T: BitStore,
    O: BitOrder,
{
...
pub fn bitslice_from_raw_parts_mut<T, O>(ptr: BitPtr<Mut, T, O>, len: usize) -> *mut BitSlice<T, O>
where
    T: BitStore,
    O: BitOrder,
{
