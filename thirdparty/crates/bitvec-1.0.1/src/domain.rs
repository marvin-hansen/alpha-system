...
use wyz::{
    comu::{Address, Const, Mut, Mutability, Reference, Referential, SliceReferential},
    fmt::FmtForward,
};
    ...
    Reference<'a, M, BitSlice<T::Unalias, O>>: Debug,
{
    ...
    fn fmt(&self, fmt: &mut Formatter) -> fmt::Result {
        ...
        match self {
            Self::Enclave(elem) => fmt.debug_tuple("Enclave").field(elem).finish(),
    ...
    Address<M, [T::Unalias]>: SliceReferential<'a>,
{
    ...
    where
        ...
        Address<M, BitSlice<T::Unalias, O>>: Referential<'a>,
        Reference<'a, M, BitSlice<T, O>>: Default,
        Reference<'a, M, BitSlice<T::Unalias, O>>: TryFrom<Reference<'a, M, [T::Unalias]>>,
    {
        match self {
            Self::Enclave(elem) => BitDomain::Enclave(elem.into_bitslice()),
            Self::Region { head, body, tail } => BitDomain::Region {
                head: head.map_or_else(Default::default, PartialElement::into_bitslice),
                ...
                tail: tail.map_or_else(Default::default, PartialElement::into_bitslice),
            },
        }
    }
}
...
where
    ...
    O: BitOrder,
    Address<M, T>: Referential<'a>,
    Address<M, [T::Unalias]>: SliceReferential<'a, ElementAddr = Address<M, T::Unalias>>,
    Address<M, BitSlice<T, O>>: Referential<'a>,
    Reference<'a, M, [T::Unalias]>: Default,
{
        ...
        BitSpan<M, T, O>: From<Reference<'a, M, BitSlice<T, O>>>,
    {
        ...
        let (head, elts, tail) = (bitspan.head(), bitspan.elements(), bitspan.tail());
    ...
    #[inline]
    fn empty(_: Address<M, T>, _: usize, _: BitIdx<T::Mem>, _: BitEnd<T::Mem>) -> Self {
        Default::default()
    }
    ...
    #[inline]
    fn major(addr: Address<M, T>, elts: usize, head: BitIdx<T::Mem>, tail: BitEnd<T::Mem>) -> Self {
        let h_elem = addr;
        let t_elem = unsafe { addr.add(elts - 1) };
        let body = unsafe {
            Address::<M, [T::Unalias]>::from_raw_parts(addr.add(1).cast::<T::Unalias>(), elts - 2)
        };
        Self::Region {
    ...
    #[inline]
    fn minor(addr: Address<M, T>, _: usize, head: BitIdx<T::Mem>, tail: BitEnd<T::Mem>) -> Self {
        let elem = addr;
    ...
    fn partial_head(
        ...
        let elem = addr;
        let body = unsafe {
            Address::<M, [T::Unalias]>::from_raw_parts(addr.add(1).cast::<T::Unalias>(), elts - 1)
        };
        Self::Region {
    ...
    fn partial_tail(
        ...
        let body = unsafe {
            Address::<M, [T::Unalias]>::from_raw_parts(addr.cast::<T::Unalias>(), elts - 1)
        };
        Self::Region {
            head: None,
            body,
    ...
    /// `body`.
    #[inline]
    fn spanning(addr: Address<M, T>, elts: usize, _: BitIdx<T::Mem>, _: BitEnd<T::Mem>) -> Self {
        Self::Region {
            head: None,
            body: unsafe {
    ...
    O: BitOrder,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::Enclave(elem) => elem.load_value().tap(|_| *self = Default::default()).into(),
...
mod tests {
    ...
    fn bit_domain() {
        ...
        let (head, body, tail) = BitDomain::<Const, usize, Lsb0>::default().region().unwrap();
        assert!(head.is_empty());
    ...
    fn render() {
        ...
        let partial = 0x3Cu8.view_bits::<Lsb0>()[2..6].domain().enclave().unwrap();
