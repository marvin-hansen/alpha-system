    ...
    O: BitOrder,
{
    ...
    pub fn copy_from_bitslice(&mut self, src: &Self) {
        ...
        let (to_head, from_head) = (self.as_bitspan().head(), src.as_bitspan().head());
        ...
        if let (Some(this), Some(that)) = (self.coerce_mut::<T, Lsb0>(), src.coerce::<T, Lsb0>()) {
            return this.sp_copy_from_bitslice(that);
        }
        if let (Some(this), Some(that)) = (self.coerce_mut::<T, Msb0>(), src.coerce::<T, Msb0>()) {
            return this.sp_copy_from_bitslice(that);
        }
        for (to, bit) in self.as_mut_bitptr_range().zip(src.iter().by_vals()) {
            unsafe {
    ...
    O: BitOrder,
{
            ...
            Domain::Region { head, body, tail } => {
                head.map_or(0, |elem| elem.load_value().count_ones() as usize)
                    + body
                        ...
                        .sum::<usize>()
                    + tail.map_or(0, |elem| elem.load_value().count_ones() as usize)
            }
        }
    }
    ...
    pub fn count_zeros(&self) -> usize {
        match self.domain() {
            Domain::Enclave(elem) => {
                (elem.load_value() | !elem.mask().into_inner()).count_zeros() as usize
            }
            Domain::Region { head, body, tail } => {
                head.map_or(0, |elem| {
                    (elem.load_value() | !elem.mask().into_inner()).count_zeros() as usize
    ...
    O: BitOrder,
{
    ...
    /// unchanged.
    pub(crate) unsafe fn unalias_mut(this: &mut BitSlice<T::Alias, O>) -> &mut Self {
