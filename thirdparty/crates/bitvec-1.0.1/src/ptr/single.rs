...
use super::{
    check_alignment, AddressExt, BitPtrRange, BitRef, BitSpan, BitSpanError, MisalignError,
};
use crate::{
    access::BitAccess,
    devel as dvl,
    ...
    O: BitOrder,
{
    ...
    /// descriptor.
    pub(crate) fn span(self, bits: usize) -> Result<BitSpan<M, T, O>, BitSpanError<T>> {
    ...
    O: BitOrder,
{
    ...
    pub fn from_slice(slice: &[T]) -> Self {
        unsafe { Self::new_unchecked(slice.as_ptr().into_address(), BitIdx::MIN) }
    }
    ...
    O: BitOrder,
{
    ...
    pub fn from_mut_slice(slice: &mut [T]) -> Self {
        unsafe { Self::new_unchecked(slice.as_mut_ptr().into_address(), BitIdx::MIN) }
    }
    ...
    O: BitOrder,
{
    ...
    where
        U: BitStore,
    {
        let (addr, head, _) = unsafe { self.span_unchecked(1) }.cast::<U>().raw_parts();
    ...
    O: BitOrder,
{
    ...
    pub(crate) unsafe fn frozen_write_bit(self, value: bool) -> bool {
        (*self.ptr.cast::<T::Access>().to_const()).write_bit::<O>(self.bit, value)
    }
}
    ...
    O: BitOrder,
{
    ...
    fn cmp(&self, other: &Self) -> cmp::Ordering {
        self.partial_cmp(other)
            .expect("BitPtr has a total ordering when type parameters are identical")
    }
}
    ...
    O: BitOrder,
{
    ...
    fn eq(&self, other: &BitPtr<M2, T2, O>) -> bool {
        ...
        self.get_addr().to_const() as usize == other.get_addr().to_const() as usize
    ...
    O: BitOrder,
{
    ...
    fn partial_cmp(&self, other: &BitPtr<M2, T2, O>) -> Option<cmp::Ordering> {
        ...
        match (self.get_addr().to_const() as usize).cmp(&(other.get_addr().to_const() as usize)) {
            cmp::Ordering::Equal => self.bit.into_inner().partial_cmp(&other.bit.into_inner()),
            ord => Some(ord),
        }
    }
}
