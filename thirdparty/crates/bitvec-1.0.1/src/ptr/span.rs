...
use tap::Pipe;
use wyz::{
    comu::{Address, Const, Mut, Mutability, NullPtrError, Reference, Referential},
    fmt::FmtForward,
};
    ...
    O: BitOrder,
{
    ...
    pub(crate) const REGION_MAX_ELTS: usize = crate::mem::elts::<T::Mem>(Self::REGION_MAX_BITS) + 1;
}
    ...
    O: BitOrder,
{
    ...
    pub(crate) fn address(&self) -> Address<M, T> {
        Address::new(unsafe {
            NonNull::new_unchecked((self.ptr.as_ptr() as usize & Self::PTR_ADDR_MASK) as *mut T)
        })
    }
    ...
    O: BitOrder,
{
        ...
        U: BitStore,
    {
        ...
        let mid_base = this
            ...
            .address()
            .cast::<U>()
            .pipe(|addr| BitPtr::<M, U, O>::new_unchecked(addr, BitIdx::MIN));
        ...
        let right_base = mid_base
            .address()
            .add(mid_elts)
            .cast::<T>()
            .pipe(|addr| BitPtr::<M, T, O>::new_unchecked(addr, BitIdx::MIN));
    ...
    pub(crate) fn from_bitslice_ptr_mut(raw: *mut BitSlice<T, O>) -> Self {
        let BitSpan { ptr, len, .. } = BitSpan::from_bitslice_ptr(raw as *const BitSlice<T, O>);
        Self {
            ptr,
            len,
            ..Self::EMPTY
    ...
    T: BitStore,
{
    ...
    fn fmt(&self, fmt: &mut Formatter) -> fmt::Result {
        ...
        match self {
            ...
            Self::Misaligned(err) => fmt.debug_tuple("Misaligned").field(&err).finish(),
            Self::TooLong(len) => fmt.debug_tuple("TooLong").field(len).finish(),
            Self::TooHigh(addr) => fmt.debug_tuple("TooHigh").field(addr).finish(),
        }
    }
}
