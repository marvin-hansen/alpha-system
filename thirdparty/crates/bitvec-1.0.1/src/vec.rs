    ...
    O: BitOrder,
{
    ...
    pub fn repeat(bit: bool, len: usize) -> Self {
        ...
        unsafe {
            out.set_len(len);
            out.as_raw_mut_slice()
                .fill_with(|| BitStore::new(if bit { !<T::Mem>::ZERO } else { <T::Mem>::ZERO }));
        }
        out
    }
    ...
    pub fn from_vec(vec: Vec<T>) -> Self {
        Self::try_from_vec(vec).expect("vector was too long to be converted into a `BitVec`")
    }
    ...
    O: BitOrder,
{
    ...
    pub fn as_raw_mut_slice(&mut self) -> &mut [T] {
        let (data, len) = (self.bitspan.address().to_mut(), self.bitspan.elements());
    ...
    pub fn into_boxed_bitslice(self) -> BitBox<T, O> {
        ...
        let mut boxed = self.into_vec().into_boxed_slice().pipe(ManuallyDrop::new);
        unsafe {
    ...
    pub fn into_vec(self) -> Vec<T> {
        ...
        mem::forget(self);
        unsafe { Vec::from_raw_parts(bitspan.address().to_mut(), bitspan.elements(), capacity) }
    }
}
    ...
    O: BitOrder,
{
    ...
    #[inline]
    fn do_reservation(&mut self, additional: usize, func: impl FnOnce(&mut Vec<T>, usize)) {
        let len = self.len();
        ...
        let new_elts = crate::mem::elts::<T>(head.into_inner() as usize + new_len);
