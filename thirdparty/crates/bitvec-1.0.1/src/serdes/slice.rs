...
use super::{utils::TypeName, Field, FIELDS};
#[cfg(feature = "alloc")]
...
use crate::{
    ...
    mem::bits_of,
    order::BitOrder,
    ptr::{AddressExt, BitSpan, BitSpanError},
    slice::BitSlice,
    store::BitStore,
};
    ...
    O: BitOrder,
{
        ...
        D: Deserializer<'de>,
    {
        deserializer.deserialize_struct(
            "BitSeq",
            FIELDS,
            BitSeqVisitor::<u8, O, &'de [u8], Self, _>::new(|data, head, bits| unsafe {
    ...
    Vec<T>: Deserialize<'de>,
{
        ...
        D: Deserializer<'de>,
    {
        deserializer.deserialize_struct(
            "BitSeq",
            FIELDS,
            BitSeqVisitor::<T, O, Vec<T>, Self, _>::new(|vec, head, bits| unsafe {
                let addr = vec.as_ptr().into_address();
                let mut bv = BitVec::try_from_vec(vec)
                    .map_err(|_| BitSpan::<Const, T, O>::new(addr, head, bits).unwrap_err())?;
                bv.set_head(head);
                bv.set_len(bits);
                Ok(bv)
            }),
...
impl<'de, T, O, In, Out, Func> Visitor<'de> for BitSeqVisitor<T, O, In, Out, Func>
where
    T: 'de + BitStore,
    O: BitOrder,
    In: Deserialize<'de>,
