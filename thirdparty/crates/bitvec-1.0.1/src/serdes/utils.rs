    ...
    R: BitRegister,
{
    ...
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_struct("BitIdx", FIELDS, BitIdxVisitor::<R>::THIS)
    }
}
    ...
    R: BitRegister,
{
        ...
        E: Error,
    {
        ...
        if width != bits_of::<R>() as u8 {
            return Err(E::invalid_type(Unexpected::Unsigned(width as u64), &self));
        }

        //  Capture an invalid index value and route it to the error handler.
        BitIdx::<R>::new(index)
            .map_err(|_| E::invalid_value(Unexpected::Unsigned(index as u64), &self))
    }
}
...
mod tests {
    ...
    fn array_wrapper() {
        ...
        assert_de_tokens_error::<Array<u32, 2>>(tokens, "invalid length 1, expected a [u32; 2]");
    }
