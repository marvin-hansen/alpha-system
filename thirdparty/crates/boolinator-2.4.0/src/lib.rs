...
mod tests {
    ...
    fn test_and_option() {
        // Such original.
        assert_eq!(
            true.and_option(Some("fries with that")),
    ...
    fn test_and_option_from() {
        // Such original.
        assert_eq!(
            true.and_option_from(|| Some("chips too, guv'")),
            Some("chips too, guv'")
        );
    ...
    fn test_as_result_from() {
        // Code good.
        assert_eq!(
            true.as_result_from(|| "four space indent", || "anything else"),
            Ok("four space indent")
        );
        assert_eq!(
            false.as_result_from(|| "four space indent", || "anything else"),
            Err("anything else")
        );
    }
