...
mod client_hello {
    ...
    impl CompleteClientHelloHandling {
        ...
        pub(in crate::server) fn handle_client_hello(
            ...
            let groups_ext = client_hello.get_namedgroups_extension().ok_or_else(|| {
            ...
            let shares_ext = client_hello.get_keyshare_extension().ok_or_else(|| {
                cx.common.send_fatal_alert(
                    AlertDescription::HandshakeFailure,
            ...
            let chosen_share_and_kxg = self.config.provider.kx_groups.iter().find_map(|group| {
                shares_ext
                    .iter()
    ...
    fn emit_server_hello(
        ...
        let kx = kxgroup.start().map_err(|_| Error::FailedToGetRandomBytes)?;
