...
fn mandatory_sign_test() {
    ...
    assert_eq!(
        b"+0",
        0i8.to_lexical_with_options::<{ FORMAT }>(&mut buffer, &options)
    );
    assert_eq!(
        b"-1",
        (-1i8).to_lexical_with_options::<{ FORMAT }>(&mut buffer, &options)
    );
    assert_eq!(
        b"+1",
        1i8.to_lexical_with_options::<{ FORMAT }>(&mut buffer, &options)
...
fn i32_test() {
    ...
    assert_eq!(
        b"-2147483648",
        (2147483648u32 as i32).to_lexical(&mut buffer)
    );
...
fn u64_test() {
    ...
    assert_eq!(
        ...
        9223372036854775807u64.to_lexical(&mut buffer)
    );
    assert_eq!(
        b"9223372036854775808",
        9223372036854775808u64.to_lexical(&mut buffer)
    );
    assert_eq!(
        b"18446744073709551615",
        18446744073709551615u64.to_lexical(&mut buffer)
    );
    assert_eq!(
        b"18446744073709551615",
        (-1i64 as u64).to_lexical(&mut buffer)
    );
}
...
fn i64_test() {
    ...
    assert_eq!(
        b"9223372036854775807",
        9223372036854775807i64.to_lexical(&mut buffer)
    );
    assert_eq!(
        b"-9223372036854775808",
        (9223372036854775808u64 as i64).to_lexical(&mut buffer)
    );
    assert_eq!(
        b"-1",
        (18446744073709551615u64 as i64).to_lexical(&mut buffer)
    );
    assert_eq!(b"-1", (-1i64).to_lexical(&mut buffer));
}
...
fn proptest_failures_radix() {
    ...
    assert_eq!(
        b"A8",
        128u8.to_lexical_with_options::<{ FORMAT }>(&mut buffer, &options)
    );
}
...
fn options_test() {
    ...
    assert_eq!(
        b"0",
        0u8.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0u16.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0u32.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0u64.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0u128.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0i8.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0i16.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0i32.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0i64.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
    assert_eq!(
        b"0",
        0i128.to_lexical_with_options::<{ STANDARD }>(&mut buffer, &options)
    );
}
