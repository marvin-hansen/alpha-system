...
use alloc_stdlib::{HeapAlloc, HeapPrealloc};
...
fn uninitialized_stack_pool_test() {
    {
        ...
        let mut ags =
            StackAllocatedFreelist4::<u8>::new_allocator(&mut stack_global_buffer, uninitialized);
        {
...
fn uninitialized_stack_pool_free_null() {
    ...
    let mut ags =
        StackAllocatedFreelist8::<u8>::new_allocator(&mut stack_global_buffer, uninitialized);
    {
...
fn uninitialized_heap_pool_test() {
    {
        let mut heap_global_buffer =
            unsafe { HeapPrealloc::<u8>::new_uninitialized_memory_pool(6 * 1024 * 1024) };
        let mut ags =
            HeapPrealloc::<u8>::new_allocator(4096, &mut heap_global_buffer, uninitialized);
        {
            let mut x = ags.alloc_cell(9999);
            x.slice_mut()[0] = 4;
            let mut y = ags.alloc_cell(4);
...
fn uninitialized_calloc_pool_test() {
    {
        let mut calloc_global_buffer =
            unsafe { define_allocator_memory_pool!(4096, u8, [0; 200 * 1024 * 1024], calloc) };
        let mut ags = CallocAllocatedFreelist4096::<u8>::new_allocator(
            &mut calloc_global_buffer.data,
            uninitialized,
        );
        {
            let mut x = ags.alloc_cell(9999);
...
fn heap_pool_test() {
    {
        let mut heap_global_buffer =
            define_allocator_memory_pool!(4096, u8, [0; 6 * 1024 * 1024], heap);
