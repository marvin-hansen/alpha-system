...
macro_rules! SaveErrorCode {
  ($state: expr, $e:expr) => {
    match assign_error_code(&mut $state.error_code, $e) {
      ...
      BrotliDecoderErrorCode::BROTLI_DECODER_NEEDS_MORE_OUTPUT => BrotliResult::NeedsMoreOutput,
...
fn DecodeSymbol(bits: u32, table: &[HuffmanCode], br: &mut bit_reader::BrotliBitReader) -> u32 {
  ...
  if table_element.bits > HUFFMAN_TABLE_BITS as u8 {
    ...
    table_element = fast!(
      (table)[(table_index + ((bits >> HUFFMAN_TABLE_BITS) & bit_reader::BitMask(nbits as u32)))
        as usize]
    );
  }
      ...
      BrotliRunningHuffmanState::BROTLI_STATE_HUFFMAN_COMPLEX => {
          ...
          .enumerate()
        {
          *next_symbol_mut = i as i32 - (max_code_length as i32);
          fast_mut!(
            (s.symbols_lists_array)
              [(s.symbol_lists_index as i32 + i as i32 - (max_code_length as i32)) as usize]
          ) = 0xFFFF;
        }
...
>(
  ...
  for htree_iter in
    fast_mut!((htrees.slice_mut())[s.htree_index as usize ; (group_num_htrees as usize)]).iter_mut()
  {
...
>(
  ...
  s.dist_htree_index =
    fast_slice!((s.dist_context_map)[s.dist_context_map_slice_index + s.distance_context as usize]);
  true
}
...
>(
  ...
  let max_dict_size = s.ringbuffer_size as usize - 16;
  {
    ...
    if (is_last != 0) {
      while (s.ringbuffer_size >= (s.custom_dict_size + s.meta_block_remaining_len) * 2
...
>(
  ...
  if (s.distance_code == 0) {
    ...
    let mut v = (s.dist_rb_idx as i32
      + (kDistanceShortCodeIndexOffset as i32 >> distance_code as i32)) as i32
      & 0x3;
...
>(
  ...
  if (distval >= 0) {
    ...
    if (!safe && (s.distance_postfix_bits == 0)) {
      ...
      s.distance_code = ((offset + bits as i32) << s.distance_postfix_bits)
        .wrapping_add(postfix)
        .wrapping_add(s.num_direct_distance_codes as i32);
    }
  }
  s.distance_code = s
    .distance_code
        ...
        BrotliRunningState::BROTLI_STATE_COMMAND_INNER => {
          ...
          if (s.trivial_literal_context != 0) {
            ...
            loop {
              ...
              fast_mut!((s.block_type_length_state.block_length)[0]) -= 1;
              BROTLI_LOG_UINT!(
                s.context_map.slice()[s.context_map_slice_index as usize + context as usize]
              );
    ...
    loop {
      ...
      match s.state {
        ...
        BrotliRunningState::BROTLI_STATE_LARGE_WINDOW_BITS => {
          ...
          if (s.window_bits < kBrotliLargeMinWbits || s.window_bits > kBrotliLargeMaxWbits) {
