...
fn test_parse_4digits() {
    assert_eq!(
        algorithm::parse_4digits::<{ STANDARD }>(0x31_32_33_34),
        4321
    );
    #[cfg(feature = "radix")]
    assert_eq!(
        algorithm::parse_4digits::<{ from_radix(5) }>(0x31_32_33_34),
        586
    );
    assert_eq!(
        algorithm::parse_4digits::<{ STANDARD }>(0x36_37_38_39),
        9876
    );
}
...
fn test_parse_8digits() {
    ...
    #[cfg(feature = "power-of-two")]
    assert_eq!(
        algorithm::parse_8digits::<{ from_radix(8) }>(value),
        2738385
    );
    ...
    {
        assert_eq!(
            algorithm::parse_8digits::<{ from_radix(9) }>(value),
            6052420
        );
        assert_eq!(
            algorithm::parse_8digits::<{ from_radix(7) }>(value),
            1120400
        );
