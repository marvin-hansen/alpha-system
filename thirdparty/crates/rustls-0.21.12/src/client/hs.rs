...
fn emit_client_hello_for_retry(
    ...
    input.hello.sent_extensions = exts.iter().map(ClientExtension::get_type).collect();
...
pub(super) fn sct_list_is_invalid(scts: &[Sct]) -> bool {
    scts.is_empty() || scts.iter().any(|sct| sct.as_ref().is_empty())
}
...
impl ExpectServerHelloOrHelloRetryRequest {
    ...
    fn handle_hello_retry_request(
        ...
        let may_send_sct_list = self.next.input.hello.server_may_send_sct_list();
