...
impl PatternTrait for LongPattern {
    ...
    fn update_patterns(&mut self, window: &[OHLCVBar; 6]) -> Result<(), String> {
        ...
        // 14 in BasePattern
        self.arr[3] = closed0 > closed2;

        // Custom pattern
        self.arr[4] = closed0 > closed3;

        // 19 in BasePattern
        self.arr[5] = closed1 > (closed2 + closed2 * POINT_FIVE / ONE_HUNDRED);

        // 20 in BasePattern
        self.arr[6] = highd0 > highd1;

        // 21 in BasePattern
        self.arr[7] = highd0 > highd5;

        // Custom pattern
        self.arr[8] = highd0 > highd1 && highd0 > highd2;
