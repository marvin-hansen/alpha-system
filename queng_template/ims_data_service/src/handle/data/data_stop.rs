/*
 * Copyright (c) "2025" . Marvin Hansen All Rights Reserved.
 */

use crate::service::Service;
use common_errors::MessageProcessingError;
use common_exchange::ExchangeID;
use sbe_types::{DataErrorType, DataType};
use trait_data_integration::ImsDataIntegration;

impl<Integration: ImsDataIntegration + 'static> Service<Integration> {
    pub async fn stop_data(
        &self,
        client_id: u16,
        data_type: &DataType,
        exchange_id: &ExchangeID,
        symbols: &[String],
    ) -> Result<(), (DataErrorType, MessageProcessingError)> {
        self.dbg_print("stop_data");

        // Verify request i.e. client logged in, exchange id, and all symbols are valid
        self.dbg_print("Verify stop_data request ");
        match self
            .verify_data_request(client_id, exchange_id, symbols)
            .await
        {
            Ok(_) => {}
            Err((error_type, err)) => return Err((error_type, err)),
        };

        let integration = self.ims_integration().read().await;

        self.dbg_print("Stop data for symbols");
        match data_type {
            DataType::UnknownDataType => {
                return Err((
                    DataErrorType::DataStopError,
                    MessageProcessingError("Unknown data type".to_string()),
                ));
            }
            DataType::TradeData => {
                self.dbg_print("Stop trade data for symbols");
                match integration.stop_trade_data(symbols).await {
                    Ok(_) => {}
                    Err(e) => {
                        return Err((
                            DataErrorType::DataStopError,
                            MessageProcessingError(e.to_string()),
                        ));
                    }
                }
            }
            DataType::OHLCVData => {
                self.dbg_print("Stop ohlcv data for symbols");
                match integration.stop_ohlcv_data(symbols).await {
                    Ok(_) => {}
                    Err(e) => {
                        return Err((
                            DataErrorType::DataStopError,
                            MessageProcessingError(e.to_string()),
                        ));
                    }
                }
            }
        }

        drop(integration);

        Ok(())
    }
}
