...
fn get_stdin_data() -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    buf.append(&mut "Hello world!".as_bytes().to_vec());
    Ok(buf)
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    let remote_addr: SocketAddr = env::args()
        .nth(1)
        .unwrap_or_else(|| "127.0.0.1:4040".into())
        .parse()?;
    ...
    let local_addr: SocketAddr = if remote_addr.is_ipv4() {
        ...
        "[::]:0"
    }.parse().expect("Failed to parse address");

    let socket = UdpSocket::bind(local_addr).await.expect("Failed to bind to address");
    const MAX_DATAGRAM_SIZE: usize = 65_507;
    socket.connect(&remote_addr).await.expect("Failed to connect to address");
