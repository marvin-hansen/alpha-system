...
async fn test_binance_data() {
    ...
    dbg!("Test client data stream");
    test_client_data_stream().await;

    dbg!("Stop and remove postgres container");
    let result = docker_util.stop_container(&pg_container_id);
    dbg!(&result);
    assert!(result.is_ok());

    dbg!("Stop and remove iggy container");
...
async fn test_client_data_stream() {}
